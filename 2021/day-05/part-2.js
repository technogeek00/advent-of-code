...
let counts = lines.reduce((counts, line) => {
    let { x1, y1, x2, y2 } = line;

    let pointName, dir, diff;
    // post-game note, you could refactor this more if you desired
    if(x1 == x2) { // vertical
        dir = Math.sign(y2 - y1);
        diff = Math.abs(y2 - y1);
        ...
        diff = Math.abs(x2 - x1); // note 45 means even x/y step
        xDir = Math.sign(x2 - x1);
        yDir = Math.sign(y2 - y1);
        pointName = (i) => `${x1 + (xDir * i)}-${y1 + (yDir * i)}`;
    }
